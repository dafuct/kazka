...
export function ChildProfileForm({ initial, onSubmit, submitLabel }: ChildProfileFormProps) {
  ...
  function addInterest() {
    ...
    setV({ ...v, interests: [...v.interests, x] })
    setInterestDraft('')
  }
  ...
  return (
    <form onSubmit={submit} className={styles.form}>
      <label className={styles.field}>
        ...
        <input value={v.name} onChange={e => setV({ ...v, name: e.target.value })} maxLength={80} required />
      </label>
      <label className={styles.field}>
        <span>{tc.fieldBirthYear ?? 'Birth year'}</span>
        <input type="number" min={1990} max={new Date().getFullYear()}
               value={v.birthYear} onChange={e => setV({ ...v, birthYear: e.target.value === '' ? '' : Number(e.target.value) })} />
      </label>
      <label className={styles.field}>
        <span>{tc.fieldGender ?? 'Gender'}</span>
        <select value={v.gender} onChange={e => setV({ ...v, gender: e.target.value as ChildFormValues['gender'] })}>
      ...
      <label className={styles.field}>
        ...
        <select value={v.preferredLanguage}
                onChange={e => setV({ ...v, preferredLanguage: e.target.value as ChildFormValues['preferredLanguage'] })}>
        ...
        <div className={styles.chipRow}>
          {v.interests.map(x => (
            <span key={x} className={styles.chip}>
              {x}
              <button type="button" aria-label="remove" onClick={() => setV({ ...v, interests: v.interests.filter(i => i !== x) })}>×</button>
            </span>
          ))}
        </div>

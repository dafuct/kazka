...
We **don't** store card details. Payment is handled by one of the providers — Apple, Paddle, LiqPay, Monobank. On our side we only keep an entitlement record: which plan is active, until when, via which provider.

...
class ModerationJudgeClientTest {
    ...
    void should_returnAllowed_when_judgeReturnsSafe() {
        stubGuard("safe");
        ModerationResult result = client.classify("uk", "пригоди трьох ведмежат", List.of("Sofia"));
    ...
    void should_returnRefusedSexual_when_judgeReturnsUnsafeS1() {
        stubGuard("unsafe\nS1");
        ModerationResult result = client.classify("uk", "оголена принцеса", List.of());
        assertThat(result).isInstanceOf(ModerationResult.Refused.class);
    ...
    void should_returnRefusedDeath_when_judgeReturnsUnsafeS8() {
        stubGuard("unsafe\nS8");
        ModerationResult result = client.classify("en", "the dragon dies", List.of());
        assertThat(((ModerationResult.Refused) result).category()).isEqualTo(ModerationCategory.DEATH);
    }
    ...
    void should_pickHighestSeverity_when_judgeReturnsMultipleCategories() {
        ...
        stubGuard("unsafe\nS8,S1");
        ModerationResult result = client.classify("en", "x", List.of());
        assertThat(((ModerationResult.Refused) result).category()).isEqualTo(ModerationCategory.SEXUAL);
    }

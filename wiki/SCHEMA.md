...
- `lesson` (in `wiki/lessons/`) — postmortems and pattern-to-repeat / pattern-to-avoid notes. One short page per lesson. Consulted before any feature work or bugfix.

Add additional types here as the wiki evolves.

### Lesson page structure

Each `lesson` page has the following body sections in order. Keep them short — a lesson page should fit on one screen.

1. **What went wrong** — one paragraph, factual, no speculation.
2. **Root cause** — the underlying reason. If unknown, say so.
3. **Fix applied** — what was actually changed. Link to the commit/PR if relevant.
4. **Pattern to use next time** — the rule that follows, phrased as a do/don't.
5. **Related** — `[[wikilinks]]` to concept pages, decisions, or sister lessons.

Frontmatter additions for `lesson` pages:
- `severity`: low | medium | high
- `status`: open | mitigated | resolved
- `domain`: optional free-text tag for the area (e.g. `auth`, `mobile-iap`, `migrations`)
...
### Pre-implementation wiki check
Before implementing a feature, fixing a bug, or making a non-trivial design choice, the agent must read `wiki/index.md` and consult any relevant `lessons/`, `concepts/`, and `synthesis/` pages. Cite any that influenced the approach with `[[wikilinks]]`. If the wiki has no relevant prior art, say so explicitly so the gap is visible.

### Capture loop
When a bug is fixed, a surprise is discovered, or the user says "we already tried that" / "this broke us before" / "don't do X" — file a `lesson` page immediately. Don't defer. Lessons are most accurate while the context is fresh.
